...
const CustomerNav: FC<CustomerNavProps> = ({}) => {
  ...
  const activeSegment = useSelectedLayoutSegment();
    return (
        ...
        <li>
          <Link href="/dashboard/requestService" className="relative flex flex-row items-center h-11 focus:outline-none hover:bg-gray-50 text-gray-600 hover:text-gray-800 border-l-4 border-transparent hover:border-indigo-500 pr-6">
            <span className="inline-flex justify-center items-center ml-4">
            <PhoneIcon className='w-6 h-6 text-gray-700'/>
            </span>
            <span className="ml-2 text-sm tracking-wide truncate">Request Service</span>
          </Link>
        </li>

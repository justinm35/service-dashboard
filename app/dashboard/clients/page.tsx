...
const Client = async ({}) => {
    ...
    let customerData: DocumentData[] = [];
    snapshot.forEach(doc => customerData.push({id: doc.id, ...doc.data()}))


    // const jaxRef = collection(db, 'Customers/GFnS558BufBZUkWLYA22/Invoices')
    // const jaxinvoces = await getDocs(jaxRef)
    // const jaxInvooicess = []
    // jaxinvoces.forEach(doc => jaxInvooicess.push({id: doc.id, ...doc.data()}))

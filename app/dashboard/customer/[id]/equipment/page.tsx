...
const Eqipment = async ({params} : EqipmentProps) => {
  ...
  return getDocs(customerEquipmentRef)
    .then((customerEquipmentSnap ) => {
        const resEquipmentData: DocumentData[] = [];
        customerEquipmentSnap.forEach(
          (doc) => { resEquipmentData.push({id: doc.id, ...doc.data()})})
